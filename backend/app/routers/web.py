...
templates.env.globals["explorer_path"] = lambda view, value="", q="", bookmark_id=None: explorer_path(
    view=view,
    value=value,
    q=q,
...
def admin_explorer_page(
    ...
    value: str = Query(""),
    q: str = Query(""),
    bookmark_id: int | None = Query(None),
    db: Session = Depends(get_db),
) -> Response:
    if not is_web_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
...
@router.post("/admin/explorer/bookmarks/{bookmark_id}")
def update_bookmark_from_explorer(
    request: Request,
    bookmark_id: int,
    title: str = Form(...),
    notes: str = Form(""),
    ...
    new_categories: str = Form(""),
    tags: str = Form(""),
    return_to: str = Form("/admin/explorer"),
    db: Session = Depends(get_db),
) -> Response:
    if not is_web_authenticated(request):
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    redirect_url = safe_internal_path(return_to, "/admin/explorer")
    if bookmark is None:
        request.session["flash"] = "Bookmark not found."
        return RedirectResponse(url="/admin/explorer", status_code=status.HTTP_303_SEE_OTHER)

    title = " ".join(title.strip().split())
    if not title:
        request.session["flash"] = "Title is required."
        return RedirectResponse(url=redirect_url, status_code=status.HTTP_303_SEE_OTHER)
    if visibility not in VISIBILITY_VALUES:
        request.session["flash"] = "Invalid visibility."
        return RedirectResponse(url=redirect_url, status_code=status.HTTP_303_SEE_OTHER)
...
def render_explorer_page(
    ...
    *,
    ...
    view = view if view in {"all", "public", "private", "uncategorized", "category", "tag"} else "all"
    value = value.strip()
    if view not in {"category", "tag"}:
        value = ""
    q = q.strip()

    query = explorer_bookmark_query(db, view=view, value=value, q=q)
    total = query.count()
    bookmarks = (
        query.order_by(Bookmark.created_at.desc(), Bookmark.id.desc())
    ...
    selected_bookmark = select_explorer_bookmark(bookmarks, bookmark_id)
    categories = db.query(Category).order_by(Category.name.asc()).all()
    tags = db.query(Tag).order_by(Tag.name.asc()).all()
    current_path = explorer_path(
        view=view,
        value=value,
        q=q,
        bookmark_id=selected_bookmark.id if selected_bookmark else None,
    )
    ...
    return templates.TemplateResponse(
        "admin_explorer.html",
        {
            ...
            "selected_category_ids": {
                category.id for category in selected_bookmark.categories
            }
            ...
            "limit": EXPLORER_LIMIT,
            "has_more": total > len(bookmarks),
            "special_nodes": explorer_special_nodes(db, view, value, q),
            "category_nodes": explorer_category_nodes(db, view, value, q),
            "tag_nodes": explorer_tag_nodes(db, view, value, q),
...
def explorer_bookmark_query(db: Session, *, view: str, value: str, q: str):
    if view == "public":
        return build_bookmark_query(db, q=q, visibility="public")
    if view == "private":
        return build_bookmark_query(db, q=q, visibility="private")
    if view == "category":
        return build_bookmark_query(db, q=q, category=value)
    if view == "tag":
        return build_bookmark_query(db, q=q, tag=value)

    query = build_bookmark_query(db, q=q)
    if view == "uncategorized":
        query = query.filter(~Bookmark.categories.any())
    return query


def select_explorer_bookmark(bookmarks: list[Bookmark], bookmark_id: int | None) -> Bookmark | None:
...
def explorer_special_nodes(db: Session, active_view: str, active_value: str, q: str) -> list[dict]:
    all_count = db.query(func.count(Bookmark.id)).scalar() or 0
    public_count = (
        db.query(func.count(Bookmark.id)).filter(Bookmark.visibility == "public").scalar() or 0
    )
    private_count = (
        db.query(func.count(Bookmark.id)).filter(Bookmark.visibility == "private").scalar() or 0
    )
    to_watch_count = (
        db.query(func.count(Bookmark.id))
        .join(Bookmark.categories)
        .filter(Category.slug == TO_WATCH_CATEGORY_SLUG)
        .scalar()
        or 0
    )
    uncategorized_count = db.query(func.count(Bookmark.id)).filter(~Bookmark.categories.any()).scalar() or 0
    nodes = [
        ("all", "", "All", all_count),
        ("public", "", "Public", public_count),
        ("private", "", "Private", private_count),
        ("category", TO_WATCH_CATEGORY_SLUG, "To Watch", to_watch_count),
        ("uncategorized", "", "Uncategorized", uncategorized_count),
    ]
    return [
        explorer_node(
            view=view,
            ...
            label=label,
            count=count,
            active=active_view == view and active_value == value,
            q=q,
        )
        for view, value, label, count in nodes
    ]


def explorer_category_nodes(db: Session, active_view: str, active_value: str, q: str) -> list[dict]:
    return [
        explorer_node(
            view="category",
            value=row["slug"],
            ...
            active=active_view == "category" and active_value == row["slug"],
            q=q,
        )
        for row in category_label_rows(db)
    ]


def explorer_tag_nodes(db: Session, active_view: str, active_value: str, q: str) -> list[dict]:
    return [
        explorer_node(
            view="tag",
            value=row["slug"],
            label=row["name"],
            count=row["bookmark_count"],
            active=active_view == "tag" and active_value == row["slug"],
            q=q,
        )
        for row in tag_label_rows(db)
    ]
...
def explorer_node(
    *,
    ...
    return {
        ...
        "count": count,
        "active": active,
        "url": explorer_path(view=view, value=value, q=q),
    }
...
def explorer_path(
    *,
    ...
    if q:
        params.append(("q", q))
    if bookmark_id is not None:
        params.append(("bookmark_id", str(bookmark_id)))
    return "/admin/explorer?" + urlencode(params)
